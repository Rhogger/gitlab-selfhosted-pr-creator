...
- **Detecção de host (obrigatória antes de chamar `glab mr create`)**: a skill **NÃO** assume o `host` default do `glab` (que costuma vir como `gitlab.com`). Antes de criar o MR, extraia o host do remote do repositório e injete via env var:
  ```bash
  GITLAB_HOST_DETECTED=$(git remote get-url origin | sed -E 's#^https?://([^/:]+).*#\1#')
  ```
  Em seguida, execute o `glab` prefixado com `GITLAB_HOST=$GITLAB_HOST_DETECTED`. Isso evita que o default `gitlab.com` seja resolvido por engano em instâncias self-hosted, retornando `404 Project Not Found`.
...
- **Tratamento de falha** (após primeira tentativa com `GITLAB_HOST` derivado do remote):
  - **404 Project Not Found / host errado / repo não encontrado** → o host derivado do remote já deve ter resolvido isso. Se mesmo assim falhar, verifique se o `-R` corresponde ao namespace/projeto correto antes de pedir reconfiguração.
  - **401/403, token inválido/expirado, host não cadastrado nos `hosts:` do config** → **PARE** e oriente o usuário a executar `scripts/glab_config.sh` (seção "🚀 Configuração Rápida"). Não tente reconfigurar o `glab` por conta própria.
...
- Criar MR (sempre prefixado com `GITLAB_HOST` derivado do remote):
  ```bash
  GITLAB_HOST=$(git remote get-url origin | sed -E 's#^https?://([^/:]+).*#\1#') \
  glab mr create -R "namespace/projeto" \
    --title "[BRANCH-NAME] TIPO: Impacto principal" \
    --description "Conteúdo" \
    --assignee "seu-user" --reviewer "username"
  ```
  A flag `-R` é opcional se o `glab` já reconhecer o repo local, mas o `GITLAB_HOST` é **obrigatório** para garantir que o host correto seja usado em vez do default global.
